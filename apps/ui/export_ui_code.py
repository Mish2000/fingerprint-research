...
ROOT_DIR = Path(r"C:\fingerprint-research\apps\ui")
OUTPUT_FILE = Path(r"C:\fingerprint-research\apps\ui\ui_code_for_ai.txt")

# נתעלם מכל נתיב שמכיל אחת מהתיקיות האלו
EXCLUDED_DIR_NAMES = {
    "node_modules",
    ".git",
    ".next",
...
def is_excluded(path: Path) -> bool:
    """בודק אם הנתיב נמצא בתוך תיקייה שיש להתעלם ממנה."""
    parts_lower = {part.lower() for part in path.parts}
    return any(excluded.lower() in parts_lower for excluded in EXCLUDED_DIR_NAMES)
...
def build_directory_tree(root: Path, included_files: list[Path]) -> str:
    ...
    def walk(prefix_path: Path, indent: str = ""):
        ...
        for node in sorted(all_nodes, key=lambda p: (len(p.parts), [x.lower() for x in p.parts])):
            if node.parent == prefix_path and node not in seen:
        ...
        for i, child in enumerate(children):
            ...
            name = child.name + ("/" if (root / child).is_dir() or any(n.parent == child for n in all_nodes) else "")
            tree_lines.append(indent + connector + name)
            next_indent = indent + ("    " if is_last else "│   ")
            if any(n.parent == child for n in all_nodes):
...
def collect_files(root: Path) -> list[Path]:
    ...
    for current_root, dirnames, filenames in os.walk(root):
        ...
        # מסירים תיקיות מוחרגות כדי ש-os.walk לא ייכנס אליהן בכלל
        dirnames[:] = [d for d in dirnames if d.lower() not in {x.lower() for x in EXCLUDED_DIR_NAMES}]
...
def export_codebase(root: Path, output_file: Path) -> None:
    ...
    with output_file.open("w", encoding="utf-8", newline="\n") as out:
        ...
        out.write("Excluded directories: " + ", ".join(sorted(EXCLUDED_DIR_NAMES)) + "\n\n")
