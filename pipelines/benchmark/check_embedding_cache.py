...
def main() -> int:
    ...
    embed_dim = expected_embed_dim_for_backbone(args.backbone)
    ...
    sample_invalid_cache_files = []

    for p in paths:
        cf = cache_file_for(cache_root, p, cfg_json, args.cache_strip_prefix)
        if cf.exists():
            error = cache_entry_error(cf, backbone=args.backbone, expected_dim=embed_dim)
            if error is None:
                ready += 1
            else:
                invalid += 1
                if len(sample_invalid_cache_files) < 10:
                    sample_invalid_cache_files.append({"path": str(cf), "error": error})
        else:
            missing += 1
    ...
    summary = {
        ...
        "sample_invalid_cache_files": sample_invalid_cache_files,
        "cfg": cfg_for_key,
    }
