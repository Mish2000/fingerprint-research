...
from src.fpbench.matchers.dedicated_matcher import DedicatedMatcher, _norm_path
...
def main():
    ...
    if args.limit and args.limit > 0:
        df = balanced_limit_by_label(df, col_y, int(args.limit))
    ...
    for i, r in df.iterrows():
        ...
        # Additional diagnostics (mirrors matcher runtime semantics)
        inlier_ratio = float(getattr(res, "inlier_ratio", 0.0))
        stability_term = float(getattr(res, "stability_term", 0.0))
