...
def build_final_mask(img_u8: np.ndarray, capture: str) -> np.ndarray:
    ...
    final = ((roi > 0) & (gate > 0)).astype(np.uint8) * 255 if roi_result.is_valid else np.zeros_like(gate)
    if (not roi_result.is_valid) or (final > 0).mean() < 0.005:
...
def main():
    ...
    ap.add_argument("--manifest", type=str, default="data/manifests/nist_sd300b/manifest.csv")
    ...
    if not manifest_path.is_file():
        raise FileNotFoundError(
            ...
            "Expected something like data/manifests/nist_sd300b/manifest.csv"
        )
