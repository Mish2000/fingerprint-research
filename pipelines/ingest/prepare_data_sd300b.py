...
"""
Prepare SD300B (1000ppi) manifest + split + canonical pair bundle for Unit 1.

Inputs (required):
  --plain_dir PATH   Directory containing SD300B plain PNGs
  --roll_dir  PATH   Directory containing SD300B roll  PNGs

Outputs (default under <repo>/data/manifests/nist_sd300b/):
  - manifest.csv
  - split.json
  - stats.json
  - pairs_pos.csv
  ...
  - pairs/split_subjects.json
  - pairs_split_build.meta.json
  - sanity_report.json

Notes:
- Paths written into CSVs are absolute paths resolved on your machine.
- The canonical checked-in bundle lives under data/manifests/, while data/processed/ remains available for local experiments when requested explicitly.
"""
...
def resolve_sd300_dirs(
    repo_root: Path,
    *,
    dataset_name: str,
    ppi: int,
    plain_dir: Optional[str],
    roll_dir: Optional[str],
) -> Tuple[Path, Path]:
    def _resolve_explicit(path_str: Optional[str]) -> Optional[Path]:
        if not path_str:
            return None
        path = Path(path_str).expanduser().resolve()
        return path if path.exists() else None

    plain = _resolve_explicit(plain_dir)
    roll = _resolve_explicit(roll_dir)

    canonical_root = repo_root / 'data' / 'raw' / 'NIST' / dataset_name / 'images' / str(ppi)
    legacy_root = repo_root / 'data' / 'raw' / dataset_name / 'images' / str(ppi)
    candidate_bases = [canonical_root, legacy_root]

    if plain is None:
        plain = _first_existing([
            p / 'png' / 'plain' for p in candidate_bases
        ] + [
            p / 'plain' for p in candidate_bases
        ])
    if roll is None:
        roll = _first_existing([
            p / 'png' / 'roll' for p in candidate_bases
        ] + [
            p / 'roll' for p in candidate_bases
        ])

    if plain is None or roll is None:
        raise FileNotFoundError(
            f'Could not resolve {dataset_name} plain/roll directories under ' 
            f"{repo_root / 'data' / 'raw'}; pass --plain_dir and --roll_dir explicitly if needed."
        )
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Prepare SD300B manifest/pairs bundle for Unit 1.")
    ap.add_argument("--plain_dir", type=str, default=None, help="Path to SD300B plain images directory")
    ap.add_argument("--roll_dir", type=str, default=None, help="Path to SD300B roll images directory")
    ap.add_argument(
        "--out_dir",
        type=str,
        default=str(rr / "data" / "manifests" / dataset),
        help="Output directory (default: <repo>/data/manifests/nist_sd300b)",
    )
    ...
    plain_dir, roll_dir = resolve_sd300_dirs(
        rr,
        dataset_name='sd300b',
        ppi=int(args.ppi),
    ...
    write_json(out_dir / "pairs" / "split_subjects.json", split_meta)
