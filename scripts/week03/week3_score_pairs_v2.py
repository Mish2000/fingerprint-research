...
def extract(path_str: str, detector: str, nfeatures: int, long_edge: int) -> tuple[list[cv2.KeyPoint], np.ndarray | None, np.ndarray]:
    path = Path(path_str)
...
# Matching / scoring
# ----------------------------
def match_and_score(des1, des2, kps1, kps2, score_mode: str, ratio: float, ransac_thresh: float) -> tuple[float, int, int]:
    ...
    # affine partial tends to behave better than homography on this data
    ...
    if score_mode == "inliers_over_k":
        # normalize by min(nfeatures) proxy (matches k is unknown here; use 1500-ish by caller)
        # caller will pass nfeatures and divide later; but here keep "inliers" and caller can normalize.
        return float(inliers), inliers, matches
...
def main():
    ...
    for _, r in pairs.iterrows():
        ...
        kps1, des1, _ = extract(pa, args.detector, args.nfeatures, args.long_edge)
        kps2, des2, _ = extract(pb, args.detector, args.nfeatures, args.long_edge)

        raw_score, inliers, matches = match_and_score(
            des1, des2, kps1, kps2,
        ...
        score = raw_score
        if args.score_mode == "inliers_over_k":
            score = raw_score / float(args.nfeatures)
        ...
        rows.append({
            ...
            "inliers": int(inliers),
            "matches": int(matches),
            "k1": int(args.nfeatures),
            "k2": int(args.nfeatures),
        })
