"""
Outputs:
- score: float in [0,1] = clip(max_tentative_sim, 0..1)
...
def build_final_mask(img_u8: np.ndarray, *, capture: str) -> np.ndarray:
    ...
    if float(np.mean(final > 0)) < 0.005:
        final = gate
...
def default_ckpt_path(root: Path) -> Path:
    # prefer what exists in current tree; fallback to older variant
    p1 = root / "reports" / "week06+07" / "patch_descriptor" / "final" / "patch_descriptor_ckpt.pth"
    p2 = root / "reports" / "week06" / "patch_descriptor" / "final" / "patch_descriptor_ckpt.pth"
...
class DedicatedMatcher:
    ...
    def _load_ckpt(self, ckpt_path: Path) -> None:
        if not ckpt_path.exists():
            raise FileNotFoundError(
                ...
                f"Expected something like reports/week06+07/patch_descriptor/final/patch_descriptor_ckpt.pth"
            )
        try:
    ...
    def score_pair(
        ...
        score = float(np.clip(mean_tentative_sim, 0.0, 1.0))
