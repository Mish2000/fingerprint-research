...
def extract_fingerprint_roi(img_u8: np.ndarray) -> ROIExtractionResult:
    """
    ...
    Failure is explicit: when no plausible connected component is found,
    the returned mask is empty and ``is_valid`` is False.
    """
    ...
    if num <= 1:
        return ROIExtractionResult(
            ...
            is_valid=False,
            failure_reason="no_connected_component",
    ...
    if foreground_fraction <= 0.005:
        return ROIExtractionResult(
            mask=np.zeros_like(img_u8, dtype=np.uint8),
            is_valid=False,
            failure_reason="largest_component_too_small",
    ...
    if foreground_fraction <= 0.005:
        return ROIExtractionResult(
            mask=np.zeros_like(img_u8, dtype=np.uint8),
            is_valid=False,
            failure_reason="eroded_component_too_small",
            foreground_fraction=foreground_fraction,
        )
...
def fingerprint_roi_mask(img_u8: np.ndarray) -> np.ndarray:
    ...
    Returns the ROI mask directly, but now returns an empty mask when ROI
    extraction fails instead of treating the full image as valid.
    """
    return extract_fingerprint_roi(img_u8).mask
